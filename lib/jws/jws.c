...
lws_jws_confirm_sig(const char *in, size_t len, struct lws_jwk *jwk)
{
	...
	if (lws_genhmac_init(&ctx, args.h_type, jwk->el.e.buf,
			     jwk->el.e.len))
		return -1;
...
static int
lws_jws_sign(const char *in, size_t len, uint8_t *sig, size_t sig_len,
	     enum lws_genhash_types hash_type, struct lws_jwk *jwk)
{
	...
	if (lws_genhash_update(&hash_ctx, (uint8_t *)in, len)) {
		lws_genhash_destroy(&hash_ctx, NULL);
	...
	n = lws_genrsa_public_sign(&rsactx, digest, hash_type, sig, sig_len);
...
lws_jws_selftest(void)
{
	...
	if (lws_jwk_create(&jwk, key_jwk, strlen(key_jwk)) < 0) {
	...
	if (lws_jwk_create(&jwk, rfc7515_rsa_key, strlen(rfc7515_rsa_key))) {
	...
	n = lws_jws_sign(buf, p - buf, (uint8_t *)sig, sizeof(sig),
			 LWS_GENHASH_TYPE_SHA256, &jwk);
	if (n < 0)
		goto bail;

	/* 2.4: encode our signature in B64 and append to the payload */

	n = lws_jws_encode_section(sig, n, 0, &p, end);
	if (n < 0)
		goto bail;

	/* 2.5: confirm our signature can be verified */
